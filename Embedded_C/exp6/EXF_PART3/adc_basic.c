...
void ADC0_SEQA_IRQHandler(void) {
	...
	if(kADC_ConvSeqAInterruptFlag & ADC_GetStatusFlags(ADC0)){
	    ...
	    yt = (float)((ADCResultStruct[0].result/4095.0) * 400) - 200;
	    e = referance - yt;
		i = i_prev + ((Ki * T) / 2) * (e + e_prev);
		controlSignal = Kp * e + (Kd/T)*(e-e_prev) + i;
		e_prev = e;
		i_prev = i;
		pwmDutyCycle = (uint32_t)((controlSignal + 20000) / 400000 * 100);
	    yt = 100*yt;
	    
	      
		if((int) yt < 0 ){
			PRINTF("newADC = -%d,%d \r \n", -1*(int)yt/100, (-1*(int)yt)%100 );
		} 
		else{
			PRINTF("newADC = %d,%d \r \n", (int)yt/100, (int)yt%100 );

	 	}
		
		
		//duty = (int)((yt+20000)/40000 * 100);
		PRINTF("duty = %d \r \n ", pwmDutyCycle);
		SCTIMER_UpdatePwmDutycycle(SCT0, kSCTIMER_Out_1, pwmDutyCycle , event1);
	}
