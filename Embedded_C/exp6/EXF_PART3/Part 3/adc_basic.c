...
int main(void) {
  ...
  while (1) {
    ...
    controlSignal = Kp*e + (Kd/0.01) * (e - e_prev) + i; 
    ...
    pwmDuty = (uint16_t)((((controlSignal)*150/25)+150 )/3);  // Mapping (-25, 25) to (0, 100)
...
void clock_init(void) {    // Set up the clock source
  ...
  SYSCON->CLKOUTDIV = 200;  // Max possible divisor is 255, 1 divides by 1. MainCLK =2 * SystemCL
